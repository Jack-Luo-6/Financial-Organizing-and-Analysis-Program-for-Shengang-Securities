...
url = 'http://query.sse.com.cn/commonExcelKcb.do?sqlId=GP_ZRZ_XMLB&currStatus=&issueMarketType=1,2&commitiResult=undefined&order=updateDate|desc,stockAuditNum|desc&keyword=&auditApplyDateBegin=&auditApplyDateEnd=&isPagination=true&pageHelp.pageSize20'
headers = {
...
reorderlist = ["通过","提交注册","新受理","已受理","已问询","已回复","暂缓审议","中止（财报更新）","中止（其他事项）","终止","终止(审核不通过)","终止(未在规定时限内回复)","终止注册","不予注册","注册生效"]
...
cat1=int(df1['审核状态'].value_counts()['通过']+df1['审核状态'].value_counts()['提交注册']+df1['审核状态'].value_counts()['已回复']+df1['审核状态'].value_counts()['已受理']+df1['审核状态'].value_counts()['已问询']+df1['审核状态'].value_counts()['暂缓审议']+df1['审核状态'].value_counts()['中止（财报更新）']+df1['审核状态'].value_counts()['中止（其他事项）']+df1['审核状态'].value_counts()['新受理'])
cat11=int(df1['审核状态'].value_counts()['通过'])
cat12=int(df1['审核状态'].value_counts()['提交注册'])
cat13=int(df1['审核状态'].value_counts()['已受理'])
cat14=int(df1['审核状态'].value_counts()['已问询'])
cat15=int(df1['审核状态'].value_counts()['已回复'])
cat16=int(df1['审核状态'].value_counts()['中止（财报更新）'])
cat17=int(df1['审核状态'].value_counts()['中止（其他事项）'])
cat18=int(df1['审核状态'].value_counts()['新受理'])
cat19=int(df1['审核状态'].value_counts()['暂缓审议'])
...
cat4=int(df2['审核状态'].value_counts()['通过']+df2['审核状态'].value_counts()['提交注册']+df2['审核状态'].value_counts()['已回复']+df2['审核状态'].value_counts()['已问询']+df2['审核状态'].value_counts()['已受理'])
cat41 = int(df2['审核状态'].value_counts()['通过'])
cat42 = int(df2['审核状态'].value_counts()['提交注册'])
cat43 = int(df2['审核状态'].value_counts()['已回复'])
cat44= int(df2['审核状态'].value_counts()['已受理'])
cat45= int(df2['审核状态'].value_counts()['已问询'])
cat5=int(df2['审核状态'].value_counts()['终止'])
...
df.loc[len(df.index)] = ['', '', '通过：' + str(cat11) , '', '', '', '', '', '', '', '']
df.loc[len(df.index)] = ['', '', '提交注册：' + str(cat12) , '', '', '', '', '', '', '', '']
df.loc[len(df.index)] = ['', '', '已受理：' + str(cat13) , '', '', '', '', '', '', '', '']
df.loc[len(df.index)] = ['', '', '已问询：' + str(cat14) , '', '', '', '', '', '', '', '']
df.loc[len(df.index)] = ['', '', '已回复：' + str(cat15) , '', '', '', '', '', '', '', '']
df.loc[len(df.index)] = ['', '', '中止（其他事项）：' + str(cat17) , '', '', '', '', '', '', '', '']
df.loc[len(df.index)] = ['', '', '暂缓审议：' + str(cat19) , '', '', '', '', '', '', '', '']
df.loc[len(df.index)] = ['', '', '', '', '', '', '', '', '', '', '']
...
df.loc[len(df.index)] = ['', '', '通过：' + str(cat41) , '', '', '', '', '', '', '', '']
df.loc[len(df.index)] = ['', '', '提交注册：' + str(cat42) , '', '', '', '', '', '', '', '']
df.loc[len(df.index)] = ['', '', '已回复：' + str(cat43) , '', '', '', '', '', '', '', '']
df.loc[len(df.index)] = ['', '', '已受理：' + str(cat44) , '', '', '', '', '', '', '', '']
df.loc[len(df.index)] = ['', '', '已问询：' + str(cat45) , '', '', '', '', '', '', '', '']
df.loc[len(df.index)] = ['', '', '', '', '', '', '', '', '', '', '']
...
df.to_csv('C:/上交所再融资/上交所整理数据.csv',sep=",",index=False,encoding='utf-16')
