...
url = 'http://listing.szse.cn/api/ras/projectrends/export?bizType=2&random=0.09028290917683202'
...
reorderlist = ["上市委会议通过","中止（其他事项）","审核通过","提交注册","新受理","已受理","已问询",'已回复',"暂缓审议","中止","终止(撤回)","终止(审核不通过)","终止(未在规定时限内回复)","终止注册","不予注册","注册生效","撤销注册"]
...
cat1=int(df1['审核状态'].value_counts()['上市委会议通过']+df1['审核状态'].value_counts()['中止（其他事项）']+df1['审核状态'].value_counts()['审核通过']+df1['审核状态'].value_counts()['提交注册']+df1['审核状态'].value_counts()['已受理']+df1['审核状态'].value_counts()['已问询']+df1['审核状态'].value_counts()['已回复']+df1['审核状态'].value_counts()['中止']+df1['审核状态'].value_counts()['新受理'])
cat11=int(df1['审核状态'].value_counts()['上市委会议通过'])
cat12=int(df1['审核状态'].value_counts()['中止（其他事项）'])
cat13=int(df1['审核状态'].value_counts()['审核通过'])
cat14=int(df1['审核状态'].value_counts()['提交注册'])
cat15=int(df1['审核状态'].value_counts()['已受理'])
cat16=int(df1['审核状态'].value_counts()['已问询'])
cat17=int(df1['审核状态'].value_counts()['已回复'])
cat18=int(df1['审核状态'].value_counts()['中止'])
cat19=int(df1['审核状态'].value_counts()['新受理'])
...
cat4=int(df2['审核状态'].value_counts()['上市委会议通过']+df2['审核状态'].value_counts()['审核通过']+df2['审核状态'].value_counts()['提交注册']+df2['审核状态'].value_counts()['已问询']+df2['审核状态'].value_counts()['已回复']+df2['审核状态'].value_counts()['中止']+df2['审核状态'].value_counts()['新受理'])
cat41=int(df2['审核状态'].value_counts()['上市委会议通过'])
cat42=int(df2['审核状态'].value_counts()['中止（其他事项）'])
cat43=int(df2['审核状态'].value_counts()['审核通过'])
cat44=int(df2['审核状态'].value_counts()['提交注册'])
cat45=int(df2['审核状态'].value_counts()['已受理'])
cat46=int(df2['审核状态'].value_counts()['已问询'])
cat47=int(df2['审核状态'].value_counts()['已回复'])
cat48=int(df2['审核状态'].value_counts()['中止'])
cat49=int(df2['审核状态'].value_counts()['新受理'])
cat5=int(df2['审核状态'].value_counts()['终止(撤回)']+df2['审核状态'].value_counts()['终止注册']+df2['审核状态'].value_counts()['撤销注册'])
cat51=int(df2['审核状态'].value_counts()['终止(撤回)'])
cat52=int(df2['审核状态'].value_counts()['终止注册'])
cat53=int(df2['审核状态'].value_counts()['撤销注册'])
...
df.loc[len(df.index)] = ['', '', '审核通过：' + str(cat13) , '', '', '', '', '', '', '', '','']
df.loc[len(df.index)] = ['', '', '提交注册：' + str(cat14) , '', '', '', '', '', '', '', '','']
df.loc[len(df.index)] = ['', '', '已问询：' + str(cat16) , '', '', '', '', '', '', '', '','']
df.loc[len(df.index)] = ['', '', '已回复：' + str(cat17) , '', '', '', '', '', '', '', '','']
df.loc[len(df.index)] = ['', '', '中止：' + str(cat18) , '', '', '', '', '', '', '', '','']
df.loc[len(df.index)] = ['', '', '新受理：' + str(cat19) , '', '', '', '', '', '', '', '','']
df.loc[len(df.index)] = ['', '', '', '', '', '', '', '', '', '', '','']
...
df.loc[len(df.index)] = ['', '', '审核通过：' + str(cat43) , '', '', '', '', '', '', '', '','']
df.loc[len(df.index)] = ['', '', '提交注册：' + str(cat44) , '', '', '', '', '', '', '', '','']
df.loc[len(df.index)] = ['', '', '已问询：' + str(cat46) , '', '', '', '', '', '', '', '','']
df.loc[len(df.index)] = ['', '', '已回复：' + str(cat47) , '', '', '', '', '', '', '', '','']
df.loc[len(df.index)] = ['', '', '中止：' + str(cat48) , '', '', '', '', '', '', '', '','']
df.loc[len(df.index)] = ['', '', '新受理：' + str(cat49) , '', '', '', '', '', '', '', '','']
df.loc[len(df.index)] = ['', '', '', '', '', '', '', '', '', '', '','']
df.loc[len(df.index)] = new_col22
df.loc[len(df.index)] = ['', '', '终止（撤回）：' + str(cat51) , '', '', '', '', '', '', '', '','']
df.loc[len(df.index)] = ['', '', '终止注册：' + str(cat52) , '', '', '', '', '', '', '', '','']
df.loc[len(df.index)] = ['', '', '撤销注册：' + str(cat53) , '', '', '', '', '', '', '', '','']
df.loc[len(df.index)] = ['', '', '', '', '', '', '', '', '', '', '','']
df.loc[len(df.index)] = new_col23
df.loc[len(df.index)] = ['', '', '注册生效：' + str(cat61) , '', '', '', '', '', '', '', '','']

df.to_csv('C:/深交所再融资/深交所整理数据.csv',sep=",",index=False,encoding='utf-16')
