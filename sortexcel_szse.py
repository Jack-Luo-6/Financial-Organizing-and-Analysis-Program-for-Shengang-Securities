...
url = 'http://listing.szse.cn/api/ras/projectrends/export?bizType=1&random=0.5221649648040791'
...
reorderlist = ["上市委会议通过","提交注册","新受理","已受理","已问询","暂缓审议","中止","终止(撤回)","终止(审核不通过)","终止(未在规定时限内回复)","终止注册","不予注册","注册生效",'没受理']
...
cat1=int(df1['审核状态'].value_counts()['上市委会议通过']+df1['审核状态'].value_counts()['提交注册']+df1['审核状态'].value_counts()['已受理']+df1['审核状态'].value_counts()['已问询']+df1['审核状态'].value_counts()['暂缓审议']+df1['审核状态'].value_counts()['中止']+df1['审核状态'].value_counts()['新受理'])
...
cat16=int(df1['审核状态'].value_counts()['中止'])
...
cat4=int(df2['审核状态'].value_counts()['上市委会议通过']+df2['审核状态'].value_counts()['提交注册']+df2['审核状态'].value_counts()['已问询']+df2['审核状态'].value_counts()['暂缓审议']+df2['审核状态'].value_counts()['中止']+df2['审核状态'].value_counts()['新受理']+df2['审核状态'].value_counts()['没受理'])
cat41=int(df2['审核状态'].value_counts()['上市委会议通过'])
cat42=int(df2['审核状态'].value_counts()['提交注册'])
cat43=int(df2['审核状态'].value_counts()['已问询'])
cat44=int(df2['审核状态'].value_counts()['暂缓审议'])
cat45=int(df2['审核状态'].value_counts()['中止'])
cat46=int(df2['审核状态'].value_counts()['新受理'])
cat5=int(df2['审核状态'].value_counts()['终止(撤回)']+df2['审核状态'].value_counts()['终止注册']+df2['审核状态'].value_counts()['终止(未在规定时限内回复)']+df2['审核状态'].value_counts()['不予注册']+df2['审核状态'].value_counts()['终止(审核不通过)'])
cat51=int(df2['审核状态'].value_counts()['终止(撤回)'])
cat52=int(df2['审核状态'].value_counts()['终止注册'])
cat53=int(df2['审核状态'].value_counts()['终止(未在规定时限内回复)'])
cat54=int(df2['审核状态'].value_counts()['不予注册'])
cat55=int(df2['审核状态'].value_counts()['终止(审核不通过)'])
...
df.loc[len(df.index)] = ['', '', '新受理：' + str(cat17) , '', '', '', '', '', '', '', '']
df.loc[len(df.index)] = ['', '', '', '', '', '', '', '', '', '', '']
...
df.loc[len(df.index)] = ['', '', '中止：' + str(cat45) , '', '', '', '', '', '', '', '']
df.loc[len(df.index)] = ['', '', '新受理：' + str(cat46) , '', '', '', '', '', '', '', '']
df.loc[len(df.index)] = ['', '', '', '', '', '', '', '', '', '', '']
df.loc[len(df.index)] = new_col22
df.loc[len(df.index)] = ['', '', '终止（撤回）：' + str(cat51) , '', '', '', '', '', '', '', '']
df.loc[len(df.index)] = ['', '', '终止注册：' + str(cat52) , '', '', '', '', '', '', '', '']
df.loc[len(df.index)] = ['', '', '终止（未在规定时限内回复）：' + str(cat53) , '', '', '', '', '', '', '', '']
df.loc[len(df.index)] = ['', '', '不予注册：' + str(cat54) , '', '', '', '', '', '', '', '']
df.loc[len(df.index)] = ['', '', '终止（审核不通过）：' + str(cat55) , '', '', '', '', '', '', '', '']
df.loc[len(df.index)] = ['', '', '', '', '', '', '', '', '', '', '']
df.loc[len(df.index)] = new_col23
df.loc[len(df.index)] = ['', '', '注册生效：' + str(cat61) , '', '', '', '', '', '', '', '']


df.to_csv('C:/深交所ipo/深交所整理数据.csv',sep=",",index=False,encoding='utf-16')
