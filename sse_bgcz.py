...
url = 'http://query.sse.com.cn/commonExcelKcb.do?sqlId=GP_BGCZ_XMLB&commitiResult=undefined&issueMarketType=1,2&currStatus=&order=updateDate|desc,stockAuditNum|desc&keyword=&auditApplyDateBegin=&auditApplyDateEnd=&isPagination=true&pageHelp.pageSize20'
headers = {
...
filename = 'C:/上交所并购重组/上交所原初数据.xls'
...
reorderlist = ["通过","提交注册","新受理","已受理","已问询","已回复","中止","终止","不予注册","注册生效"]
...
cat1=int(df1['审核状态'].value_counts()['通过']+df1['审核状态'].value_counts()['提交注册']+df1['审核状态'].value_counts()['已受理']+df1['审核状态'].value_counts()['已问询']+df1['审核状态'].value_counts()['已回复']+df1['审核状态'].value_counts()['中止'])
...
cat2=int(df1['审核状态'].value_counts()['终止']+df1['审核状态'].value_counts()['不予注册'])
cat21=int(df1['审核状态'].value_counts()['终止'])
cat22=int(df1['审核状态'].value_counts()['不予注册'])
...
cat4=int(df2['审核状态'].value_counts()['已受理']+df2['审核状态'].value_counts()['已问询']+df2['审核状态'].value_counts()['中止'])
cat41 = int(df2['审核状态'].value_counts()['已受理'])
cat42 = int(df2['审核状态'].value_counts()['已问询'])
cat43 = int(df2['审核状态'].value_counts()['中止'])
...
df.loc[len(df.index)] = ['', '', '中止：' + str(cat43) , '', '', '', '', '', '', '', '']
df.loc[len(df.index)] = ['', '', '', '', '', '', '', '', '', '', '']
...
df.to_csv('C:/上交所并购重组/上交所整理数据.csv',sep=",",index=False,encoding='utf-16')
